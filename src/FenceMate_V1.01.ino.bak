...
//#include <ILI9341_t3.h>           // fast display driver lib - Teensy library (not used)
#include "SPI.h"
#include "Adafruit_GFX.h"
#include "Adafruit_ILI9341.h"
#include <XPT2046_Touchscreen.h>  // touch driver for a TFT display
#include <EEPROM.h>               // lib to talk to Arduino EEPROM
...
// Arduino UNO R4 uses Adafruit_ILI9341 driver (not Teensy's ILI9341_t3)
Adafruit_ILI9341 Display = Adafruit_ILI9341(TFT_CS, TFT_DC);
XPT2046_Touchscreen Touch(T_CS, T_IRQ);
TS_Point p;

// Custom timer to replace Teensy's elapsedMillis
...
FlickerFreePrint<Adafruit_ILI9341> GText(&Display, C_WHITE, C_BLACK);
FlickerFreePrint<Adafruit_ILI9341> CText(&Display, C_WHITE, C_BLACK);
FlickerFreePrint<Adafruit_ILI9341> SText(&Display, C_WHITE, C_BLACK);

void setup() {

  Serial.begin(115200);  // UNO R4 uses standard 115200, but 38400 also supported
...
void SlewFence (long Steps, bool UpdateAbs) {
  ...
  if (Steps > 0) {
    ...
    for (i = 0; i < Steps; i++) {
      ...
      if ((LiveUpdate) && ((millis() - DisplayUpdateTime) > DISPLAY_UPDATE)) {
        DisplayUpdateTime = millis();
        UpdateAbsoluteCoord(true);
      }
